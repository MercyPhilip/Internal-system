...
CommentsDivJs.prototype = {
	...
	,_getComments: function (pageNo, resultDivId, btn) {
		...
		tmp.ajax = new Ajax.Request('/ajax/getComments', {
			method: 'get'
			,parameters: {'entity': tmp.me._entityName, 'entityId': tmp.me._entityId, 'orderBy': {'created':'desc'}, 'pageNo': pageNo, 'pageSize': tmp.me._pageSize, 'storeId' : jQuery('#storeId').attr('value'), 'userId' : jQuery('#userId').attr('value')}
			,onCreate: function() {
				if(tmp.pageNo === 1) {
