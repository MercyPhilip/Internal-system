	...
	public function sendEmail($sender, $param)
	{
		...
		try
		{
			...
			foreach($emailLists as $emailAddress)
			{
				...
				EmailSender::addEmail($replyEmailAddress, $emailAddress, Config::get('PDFInvoice','BankDetail')['AccName'] . ' ' . $type. ':' . $order->getOrderNo() , (trim($emailBody) === '' ? '' : $emailBody . "<br /><br />") .'Please find attached ' . $type. ' (' . $order->getOrderNo() . '.pdf) from ' . Config::get('PDFInvoice','CompanyDetail')['Name'] . '.', array($asset));
			}
