...
PageJs.prototype = Object.extend(new CRUDPageJs(), {
	_getTitleRowData: function() {
		return {'description': "Description", 'name': 'Name', 'allowMultiple' : 'multi?'};
	}
	...
	,_getEditPanel: function(row) {
		...
		tmp.newDiv = new Element('tr', {'class': 'save-item-panel info'}).store('data', row)
			...
			.insert({'bottom': new Element('td', {'class': 'form-group'})
				.insert({'bottom': new Element('input', {'type': 'checkbox', 'class': 'form-control', 'save-item-panel': 'allowMultiple', 'checked': row.allowMultiple}) })
			})
