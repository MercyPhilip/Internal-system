...
PageJs.prototype = Object.extend(new BPCPageJs(), {
	...
	,genPage: function(table, pageNo, totalPages, rows) {
		...
		if(pageNo != totalPages){
			tmp.margin = 'margin-bottom:' + (tmp.me._pixels - rows[rows.size() - 1]) + 'px'; 
		}else{
			tmp.margin = 'margin-bottom:' + 0 + 'px';
		}
...
//			for(tmp.j = tmp.me._maxRowsPerPage * 1 - rows.size(); tmp.j--;) {
			for(tmp.j = Math.floor((tmp.me._pixels - rows[rows.size() - 1]) / 36); tmp.j--;){
...
//		});
	...
	,formatForPDF: function() {
		...
		tmp.mainTable.down('tbody').getElementsBySelector('tr').each(function(row) {
			...
			tmp.numDesc = Math.ceil(tmp.lenDesc / 50);
			if (tmp.numDesc >= tmp.numName){
				tmp.linepixels = 15 * tmp.numDesc + 10 * 1;
			}else{
				tmp.linepixels = 15 * tmp.numName + 10 * 1;
			}
