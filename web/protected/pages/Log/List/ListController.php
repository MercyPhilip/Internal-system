...
class ListController extends CRUDPageAbstract
{
	...
	public function getItems($sender, $param)
	{
		...
		try
		{
			...
			$where = array(1);
			$params = array();
			if(isset($serachCriteria['log.msg']) && ($type = trim($serachCriteria['log.msg'])) !== '')
			{
				$where[] = 'log.msg like ?';
				$params[] = '%' . $type . '%';
			}
			...
			$stats = array();
			$objects = $class::getAllByCriteria(implode(' AND ', $where), $params, false, $pageNo, $pageSize, array('log.id' => 'desc'), $stats);
