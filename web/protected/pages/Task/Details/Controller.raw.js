...
PageJs.prototype = Object.extend(new DetailsPageJs(), {
	...
	,_formatOrderSelection: function(order) {
		return '<div class="row order_item"><div class="col-xs-3">' + order.orderNo + '</div><div class="col-xs-2"><small>' + order.type + '</small></div><div class="col-xs-2" order_status="' + order.status.name + '"><small>' + order.status.name + '</small></div><div class="col-xs-5"><small>' + ((order.customer && order.customer.name) ? order.customer.name : '') + '</small></div></div >';
	}
	/**
	...
	,_initCustomerSearchBox: function() {
		var tmp = {};
		tmp.me = this;
		tmp.selectBox = jQuery('[save-panel="customerId"]').select2({
		...
		if(tmp.me._item.id && tmp.me._item.customer && tmp.me._item.customer.id) {
			tmp.selectBox.select2('data', {'id': tmp.me._item.customer.id, 'text': tmp.me._item.customer.name, 'data': tmp.me._item.customer});
		} else if(tmp.me._preSetData && tmp.me._preSetData.customer && tmp.me._preSetData.customer.id) {
	...
	,_initOrderSearchBox: function() {
		var tmp = {};
		tmp.me = this;
		tmp.selectBox = jQuery('[save-panel="orderId"]').select2({
			 minimumInputLength: 3
			 ,multiple: false
			 ,allowClear: true
			 ,ajax: {
		...
		if(tmp.me._item.id && tmp.me._item.fromEntity && tmp.me._item.fromEntity.id && tmp.me._item.fromEntityName === 'Order') {
			tmp.selectBox.select2('data', {'id': tmp.me._item.fromEntity.id, 'text': tmp.me._item.fromEntity.orderNo, 'data': tmp.me._item.fromEntity});
		} else if(tmp.me._preSetData && tmp.me._preSetData.order && tmp.me._preSetData.order.id) {
