		...
		tmp.newDiv = new Element('div', {'class': 'panel panel-default'})
			...
			.insert({'bottom': new Element('div', {'class': 'panel-body'})
				.insert({'bottom': new Element('div', {'class': 'row'})
					...
					.insert({'bottom': new Element('div', {'class': 'col-sm-1'}).update(tmp.me._getFormGroup('Credit Left', new Element('input', {'class': 'form-control', 'save-item': 'credit', 'validate_currency': true, 'value': tmp.item.creditpool ? tmp.me.getCurrency(tmp.item.creditpool.totalCreditLeft) : tmp.me.getCurrency(0)}) ) ) })
