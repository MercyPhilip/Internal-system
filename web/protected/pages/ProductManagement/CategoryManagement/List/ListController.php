...
class ListController extends CRUDPageAbstract
{
	...
	protected function _getEndJs()
	{
		...
		$js .= ".setCallbackId('updateSellingPrice', '" . $this->updateSellingPriceBtn->getUniqueID() . "')";
	...
	public function getItems($sender, $param)
    {
        ...
        try
        {
            ...
            foreach($results['items'] as $row)
            	$proIdMaps[$row['id']] = $row + array('lastbuyprice' => '', 'lastrecdate' => '', 'ow' => 0, 'tw' => 0, 'om' => 0);
            
            $productIds = array_keys($proIdMaps);
            // get last buy price and date
            $lastbuys = $this->getLastBuy($productIds);
     ...
     * @throws Exception
     */
    public function updateSellingPrice($sender, $param)
    {
    	$results = $errors = array();
    	try
    	{
    ...
    private function _getExcel($data)
    {
    	...
    	$activeSheet->setCellValueByColumnAndRow($columnNo++ , $rowNo, 'Last Received Date');
    	...
    	$activeSheet->setCellValueByColumnAndRow($columnNo++ , $rowNo, 'Margin');
    	...
    	foreach($data as $rowNoData)
    	{
    		...
    		$activeSheet->setCellValueByColumnAndRow($columnNo++ , $rowNo, $rowNoData['lastrecdate']);
    		...
    		$sellingPrice = 0;
    		if ( isset($rowNoData['prices']))
    		{
    			foreach($rowNoData['prices'] as $price)
    			{
    				if (isset($price['type']) && $price['type']['id'] == 1)
    		...
    		$activeSheet->setCellValueByColumnAndRow($columnNo++ , $rowNo, $sellingPrice);
    		$activeSheet->setCellValueByColumnAndRow($columnNo++ , $rowNo, $margin);
    	...
    	$fileName = 'CategoryManagement_' . str_replace(':', '_', str_replace('-', '_', str_replace(' ', '_', $now->setTimeZone(SystemSettings::getSettings(SystemSettings::TYPE_SYSTEM_TIMEZONE))))) . '.xlsx';
