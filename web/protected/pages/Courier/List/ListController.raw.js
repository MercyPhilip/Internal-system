...
PageJs.prototype = Object.extend(new CRUDPageJs(), {
	...
	,_getEditPanel: function(row) {
		...
		tmp.newDiv = new Element('tr', {'class': 'save-item-panel info'}).store('data', row)
			...
			.insert({'bottom': new Element('td', {'class': 'form-group'})
				.insert({'bottom': new Element('input', {'required': true, 'class': 'form-control', 'placeholder': 'Courier Name', 'save-item-panel': 'name', 'value': row.name ? row.name : ''}) })
			})
	...
	,_getResultRow: function(row, isTitle) {
		...
		tmp.row = new Element('tr', {'class': (tmp.isTitle === true ? '' : 'btn-hide-row')}).store('data', row)
			...
			.insert({'bottom': jQuery('#storeId').attr('value') != 1 ? '' :
				new Element(tmp.tag, {'class': 'text-right col-xs-2'}).update(
				tmp.isTitle === true ?  
