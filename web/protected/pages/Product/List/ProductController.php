...
class ProductController extends CRUDPageAbstract
{
	...
	private function getSearchCriteria($serachCriteria)
	{
		...
		switch(trim($serachCriteria['pro.sh']))
		{
			case 'all':
				$result->sh_from = $result->sh_to = null;
				break;
			case '=':
				$result->sh_from = $result->sh_to = isset($serachCriteria['pro.sh_num']) ? $serachCriteria['pro.sh_num'] : null;
				break;
			case '>':
				$result->sh_from = isset($serachCriteria['pro.sh_num']) ? $serachCriteria['pro.sh_num'] : null;
				$result->sh_to = null;
				break;
			case '<':
				$result->sh_from = null;
				$result->sh_to = isset($serachCriteria['pro.sh_num']) ? $serachCriteria['pro.sh_num'] : null;
				break;
			default: throw new Exception('Invalid stock on hand range, "' . $serachCriteria['pro.sh'] . $serachCriteria['pro.sh_num'].'" given');
		}
		//stock level
	...
	public function toggleSellOnWeb($sender, $param)
	{
		...
		{
			Dao::beginTransaction();
			$ids = isset($param->CallbackParameter->productIds) ? $param->CallbackParameter->productIds : '';
			if (isset($param->CallbackParameter->isSellOnWeb)){
				...
				$product->setSellOnWeb($sellOnWeb)
				->save()
				...
				foreach ($ids as $id){
					...
					if ($product->getSellOnWeb() == 1){
						...
						{
							throw new Exception("Can't take off product: " . $product->getSku() . "from online because SOH is not zero.");
						}
						$product->setSellOnWeb(0)->save()
						->addLog('SellOnWeb changed by ' . Core::getUser()->getUserName() . '(' . 1 . ' => ' . 0 . ')', Log::TYPE_SYSTEM, 'SELLONWEB_CHG', __CLASS__ . '::' . __FUNCTION__);
					}else{
						$product->setSellOnWeb(1)->save()
						->addLog('SellOnWeb changed by ' . Core::getUser()->getUserName() . '(' . 0 . ' => ' . 1 . ')', Log::TYPE_SYSTEM, 'SELLONWEB_CHG', __CLASS__ . '::' . __FUNCTION__);
					}
	...
	public function toggleActive($sender, $param)
	{
		...
		{
			Dao::beginTransaction();
			$ids = isset($param->CallbackParameter->productIds) ? $param->CallbackParameter->productIds : array();
